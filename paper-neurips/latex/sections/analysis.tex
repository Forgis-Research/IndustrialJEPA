...
\plannedc{
\subsection{Multivariate Representation Analysis}
\label{sec:multi_analysis}

\placeholder{Analysis of what multivariate encoders learn: do physics-group representations emerge? Does spatiotemporal masking force richer cross-modal representations? Visualization of learned attention patterns across sensor modalities.}
}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Single channel.} Current experiments use only one vibration channel. \plannedc{The multivariate extension (\cref{sec:multivariate}) addresses this limitation.}
  ...
  \item \textbf{Two transfer pairs.} Cross-dataset evaluation is limited to FEMTO$\leftrightarrow$XJTU-SY. \plannedc{The C-MAPSS experiments (\cref{sec:cmapss}) expand the evaluation to turbofan engines.}
\end{enumerate}
