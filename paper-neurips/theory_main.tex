...
We write $p = P(E_{t+\Delta t}=1)$ for the marginal event rate; the probability surface $p(t, \Delta t)$ in other sections is a distinct quantity.
We use $I(\cdot;\cdot)$ for mutual information, following \citet{cover2006elements}.
Entropy is denoted $\mathbb{H}(\cdot)$ to avoid confusion with the embedding $H_t$; see \cref{app:theory} for full notation.
...
\textup{(A4)} the marginal event rate satisfies $p = P(E_{t+\Delta t}=1) \in [\underline{p},\, \overline{p}] \subset (0,1)$,
and \textup{(A1')} the event posterior $\eta(h) = P(E_{t+\Delta t}=1 \mid H^*=h)$ is bounded: $\eta(H^*) \in [\underline{\eta}, \overline{\eta}] \subset (0,1)$ a.s.
Then
\begin{equation}
\label{eq:main_bound}
I(H_t;\, E_{t+\Delta t}) \;\geq\; I(H^*;\, E_{t+\Delta t}) \;-\; C_\eta \, L^2 \, \varepsilon\;,
\end{equation}
where $C_\eta = \bigl(2\,\underline{\eta}\,(1-\overline{\eta})\bigr)^{-1}$ depends on the posterior bounds from A1'.
\end{proposition}
...
A Jensen-gap argument on the convex KL divergence, combined with the calibrated-posterior bound A1', the Lipschitz bound A3, and prediction error A2, yields $I(H^*; E) - I(\hat{H}; E) \leq C_\eta L^2 \varepsilon$, where $C_\eta = (2\underline{\eta}(1-\overline{\eta}))^{-1}$ depends on the posterior bounds from \ref{ass:calibrated_posterior} in \cref{app:theory}.
...
\begin{enumerate}[nosep,label=(\roman*)]
  ...
  \item $\varepsilon < I(H^*; E_{t+\Delta t}) / (C_\eta L^2)$: the predictor approximates the target representation well enough;
...
For C-MAPSS, where $I(H_t; E_{t+\Delta t})$ is large, the statistical error term already falls below the representation gap by $n \approx 200$ windows.
