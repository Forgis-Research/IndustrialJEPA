...
\Cref{tab:transfer} shows the cross-dataset results.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Two transfer pairs.} Cross-dataset evaluation is limited to FEMTO$\leftrightarrow$XJTU-SY. Validation on additional domains (e.g., turbofan engines) would strengthen generalization claims.
\end{enumerate}
...
The XJTU-SY bearing dataset~\citep{wang2018xjtusybearing} contains 15 rolling element bearings tested under 3 operating conditions. Each bearing is monitored with two accelerometers (horizontal and vertical) at 25.6\,kHz. We use 7 bearings with clear degradation trajectories.
...
SSL (ours) & \textbf{Traj JEPA E2E} & \textbf{13.80}$_{\pm0.8}$ & \textbf{14.93}$_{\pm0.4}$ & \textbf{16.54}$_{\pm0.8}$ & \textbf{18.66}$_{\pm0.8}$ & 25.33$_{\pm5.1}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[nosep]
  \item \textbf{Beats prior SSL on C-MAPSS}: Traj JEPA E2E (13.80) outperforms AE-LSTM SSL (13.99), the only published SSL result on C-MAPSS FD001, without using failure-time labels during pretraining.
  ...
  \item \textbf{Strong representation quality}: The first principal component of frozen embeddings achieves Spearman $\rho = 0.814$ correlation with RUL, confirming that trajectory prediction implicitly learns the degradation axis without failure labels.
...
These results confirm that the JEPA framework generalizes beyond vibration signals: the trajectory prediction objective learns degradation-aware representations from unlabeled multivariate sensor streams, with strong label efficiency and stability advantages over supervised baselines.
