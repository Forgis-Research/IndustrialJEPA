...
We demonstrate the approach across industrial (C-MAPSS turbofan degradation) and spacecraft (SMAP/MSL anomaly detection) domains. On SMAP, the pretraining prediction error achieves PA-F1 = 62.5\%, surpassing MTS-JEPA (33.6\%) without anomaly labels---demonstrating that the same encoder supports diverse grey swan tasks through linear probes alone.
\end{abstract}
...
\begin{table}[t]
\centering
\caption{\textbf{Grey swan prediction benchmark.} Datasets as column groups; example grey swan events as sub-columns. Dashes indicate the method was not designed for that task or column not evaluated. $\dagger$ = single-seed or best-of-grid. Anomaly F1 = point-adjusted (PA-F1) in percentage, following literature convention; TTE columns are future work. The key pattern: every existing method fills $\leq$2 dataset groups. Our approach spans multiple domains using the same encoder and pretraining objective, with only the downstream probe changing. $\S$~=~SMAP; single seed; preliminary.}
\label{tab:benchmark}
\small
...
The non-PA F1 results are near the random baseline, confirming that point-by-point anomaly detection is difficult with a single threshold. The PA-F1 scores (62.5\% SMAP, 43.3\% MSL) are substantially higher and exceed TS2Vec and MTS-JEPA on SMAP PA-F1---demonstrating that the predictor correctly localizes anomaly \emph{segments} even when per-point precision is low. This pattern is consistent with the anomaly probe being a segment-level detector: the prediction error accumulates over anomalous windows rather than spiking at individual points.
...
Full-seq target variant & 15.70$_{\pm0.2}$ & 14.32$_{\pm0.6}$ \\
Bidirectional context (V16b) & 25.72$_{\pm1.6}$ & 15.06$_{\pm1.2}$ \\
SIGReg (no EMA)$^*$ & --- & --- \\
\bottomrule
\end{tabular}
\end{table}
