...
On the NASA C-MAPSS benchmark (FD001), a controlled from-scratch ablation isolates the pretraining contribution: $+8.8$ RMSE at full labels, growing to $+16.9$ at 10\%.
At 5\% labels, the frozen encoder ($21.53 \pm 2.0$) surpasses matched-budget supervised STAR ($24.55 \pm 6.4$) - the label-efficiency crossover that makes self-supervised pretraining the rational default in the scarce-label regime.
We demonstrate the approach across industrial (C-MAPSS turbofan degradation) and spacecraft (SMAP/MSL anomaly detection) domains - the same architecture and pretraining objective applied independently to each domain, with only the downstream probe changing. On SMAP, the prediction-error anomaly score achieves PA-F1 $= 62.5\%$ without anomaly labels (vs.\ $33.6\%$ for MTS-JEPA); we note that point-adjusted F1 is an optimistic segment-level metric and that non-PA F1 ($\approx$7\%) is close to random, reflecting a segment-localized rather than point-precise detector.
\end{abstract}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Causal inductive bias matters more than model complexity.} A bidirectional context encoder with VICReg regularisation (V16b) attains comparable supervised fine-tuning RMSE but substantially worse frozen-probe generalisation (test RMSE 25.7 vs.\ 17.8) and worse cross-machine transfer ($+4$ to $+11$ RMSE on FD002-FD004) than the causal default (\cref{fig:cross_machine}, \cref{sec:ablations}).

  \item \textbf{Quantified pretraining contribution.} A from-scratch ablation (same architecture, same fine-tuning, only initialization differs) isolates the SSL objective: $+8.8$ RMSE at full labels, growing to $+16.9$ at 10\% (\cref{sec:from_scratch}).
...
Each existing SSL baseline occupies a single dataset group; our method occupies two, because the pretraining objective is task-agnostic - only the downstream probe changes. The grey-swan framing (time-to-failure, anomaly onset, and threshold exceedance as in \cref{sec:problem}) is meant to anticipate a broader benchmark; the current table demonstrates two of the four proposed event types. \plannedc{SWaT water-treatment anomaly detection follows the identical protocol (causal JEPA pretraining on the 51-channel sensor stream with no attack labels; anomaly score from \cref{eq:anomaly_score}; threshold at the $95^\text{th}$ percentile of scores from the first 10\% of test data). The SWaT benchmark is included in the table as the third dataset group; SWaT numerical results are completed in a separate run (see \cref{app:metrics}) and will populate the final SWaT column cells.}
...
Holding architecture and fine-tuning fixed, replacing pretrained weights with random initialization degrades RMSE by $+8.8$ at 100\% ($22.99 \to 14.23$) and $+16.9$ at 10\% ($35.59 \to 18.66$). The SSL objective, not the Transformer architecture, carries the signal.
...
\paragraph{Bidirectional context - the causal inductive bias finding.} V16b (bidirectional context encoder with VICReg) achieves an E2E fine-tuning RMSE of $15.06 \pm 1.2$ (slightly worse than V2's $14.23$) but its frozen test RMSE ($25.72 \pm 1.6$) is substantially worse than V2 ($17.81$). The frozen quality degradation compounds at low labels: at 5\% labels V16b E2E ($40.6 \pm 10.2$) is substantially worse than V2 E2E ($25.3 \pm 5.1$). Cross-machine transfer also degrades uniformly (\cref{fig:cross_machine}): FD001-pretrained V16b yields $38.04$ / $37.76$ / $49.66$ on FD002 / FD003 / FD004 respectively, versus $27.68$ / $31.45$ / $38.32$ for V2 - a $+10$ to $+13$ RMSE cost for bidirectionality on every target. We interpret this as evidence that \emph{causal temporal ordering is a domain-invariant inductive bias}, while the bidirectional encoder memorises FD001-specific degradation profiles that do not transfer. Since supervised fine-tuning is the setting where V16b is least far behind (+0.83 RMSE), the pattern is consistent with bidirectionality helping memorization but hurting the transferable degradation axis.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig_cross_machine.pdf}
\caption{\textbf{Cross-machine transfer: the causal inductive bias wins on every target subset.} FD001-pretrained encoder, zero-shot frozen probe on FD002 / FD003 / FD004. V2 (causal, blue) beats V16a (bidirectional, teal) and V16b (bidirectional + VICReg, orange) by 4-12 RMSE at every target. Mean $\pm$ std over 3 probe seeds. Source: \texttt{phase4\_cross\_machine\_results.json}.}
