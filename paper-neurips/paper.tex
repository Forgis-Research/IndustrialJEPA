...
\paragraph{SIGReg variant.} We also explore replacing EMA with SIGReg spectral regularization~\citep{balestriero2025lejepa}: a single shared encoder (no target copy) with a regularization term $\loss_\text{SIG}$ that prevents collapse by maintaining spectral diversity of the representation covariance. Preliminary results show SIGReg achieves competitive frozen probe quality (V15-SIGReg verified: RMSE $= 11.1 \pm 0.8$ vs.\ V2-EMA: $17.8 \pm 1.7$) with $\sim$20\% fewer parameters (no predictor head), but its probe trajectory exhibits high-frequency oscillations that make best-checkpoint selection unstable across runs. We use the EMA variant for all main results due to its training stability.
...
A key architectural finding is that causal inductive bias matters more than representational expressiveness: a bidirectional encoder achieves better within-domain frozen probe quality but worse label efficiency and cross-domain transfer, while the causal model generalizes more robustly. We conjecture this is because causal encoders learn a temporal ordering bias that transfers across domains, while bidirectional encoders memorize domain-specific degradation patterns.
...
This pattern is FD001-specific. On FD003 (fan + HPC faults), attention shifts in the opposite direction: the top degradation changes are defocusing from physical fan speed ($s_8$) and corrected core speed ($s_{13}$), while LPC outlet temperature ($s_3$) receives increased cross-sensor attention. Both findings are consistent with the underlying fault physics. Cross-sensor attention maps for both subsets are provided as figures.
