...
% ============================================================
\title{Trajectory JEPA: Self-Supervised Grey Swan Prediction\\from Multivariate Time Series}
...
% ============================================================
\begin{abstract}
Grey swan events---rare but partially predictable failures, anomalies, and threshold breaches---are the highest-stakes prediction targets in multivariate time series, yet they are precisely the events with the least labeled data.
We introduce \textbf{Trajectory JEPA}, a Joint Embedding Predictive Architecture that learns general-purpose representations by predicting future sensor trajectories in latent space, without event labels.
A single pretrained encoder supports four grey swan event types through trivial downstream probes: remaining useful life regression (linear), anomaly detection (the pretraining prediction error itself---zero labels required), threshold exceedance prediction (linear), and fault classification (linear).
\plannedc{We evaluate across four domains---industrial (C-MAPSS, SWaT), spacecraft telemetry (SMAP, MSL), financial markets (VIX), and medical (PhysioNet Sepsis)---establishing the first unified SSL benchmark for grey swan prediction.
Every baseline fills at most two columns of our benchmark table; Trajectory JEPA fills all of them.}
On C-MAPSS FD001, a controlled from-scratch ablation isolates the pretraining contribution: $+8.8$ RMSE at full labels, $+15.6$ at 10\%.
At 5\% labels, the frozen encoder ($21.53 \pm 2.0$) beats the supervised state of the art ($24.55 \pm 6.5$)---the label-efficiency crossover that makes self-supervised pretraining the rational default in the grey swan regime.
\end{abstract}
...
\paragraph{One encoder, four event types.} After pretraining, a single frozen encoder supports multiple grey swan tasks through trivial probes (\Cref{fig:architecture}):
\begin{enumerate}[nosep]
  \item \textbf{Time-to-Failure (RUL):} linear probe $\h_t \to \hat{y}$, supervised.
  \item \textbf{Anomaly detection:} the pretraining prediction error $\|\hat{\h}_\text{fut} - \h_\text{fut}\|_1$ as anomaly score---\emph{zero labels required}.
  \item \textbf{Threshold exceedance:} linear probe $\h_t \to \hat{\tau}$, predicting time until a sensor crosses $N\sigma$, supervised.
  \item \textbf{Fault classification:} linear probe $\h_t \to K$ classes, supervised.
\end{enumerate}
...
\begin{enumerate}[nosep]
  ...
  \item \plannedc{\textbf{Unified grey swan benchmark.} Evaluation across four domains and four event types, where no prior method fills more than two columns of the benchmark table (\cref{sec:benchmark}).}

  \item \textbf{Anomaly detection without labels.} The pretraining prediction error is directly an anomaly score, providing zero-shot anomaly detection from the SSL objective alone (\cref{sec:anomaly_probe}).
\end{enumerate}
...
\textbf{Covariate handling.} Operating conditions and auxiliary variables enter through the normalization pathway (per-condition clustering) rather than as explicit input tokens, keeping the encoder architecture domain-agnostic. A cross-sensor variant (\cref{sec:cross_sensor}) instead tokenizes each sensor individually with learnable sensor-ID embeddings.
...
\begin{table}[t]
\centering
\caption{\textbf{Grey Swan Prediction Benchmark.} One architecture, one pretraining objective, four domains.
Dashes (---) indicate the method was not designed for or evaluated on that event type.
\textcolor{blue}{Blue} = planned experiments.
Best SSL result per column in \textbf{bold}. $\dagger$ = single-seed or best-of-grid.}
\label{tab:benchmark}
\small
\setlength{\tabcolsep}{3pt}
...
\toprule
& & \multicolumn{3}{c|}{Time-to-Failure (RMSE $\downarrow$)} & \multicolumn{3}{c|}{Anomaly (F1 non-PA $\uparrow$)} & TTE \\
& Method & FD001 & FD003 & FD004 & SMAP & MSL & SWaT & C-MAPSS \\
\midrule
\multicolumn{9}{@{}l}{\textit{Supervised specialists}} \\
& STAR~\citep{fan2024star}$^\dagger$ & 10.61 & 10.71 & 15.87 & --- & --- & --- & --- \\
& TTSNet~\citep{ttsnet2025} & 11.02 & 11.06 & 18.26 & --- & --- & --- & --- \\
& Anom.\ Transf.~\citep{xu2022anomalytransformer} & --- & --- & --- & \textcolor{blue}{0.XX} & \textcolor{blue}{0.XX} & \textcolor{blue}{0.XX} & --- \\
& DCdetector~\citep{yang2023dcdetector} & --- & --- & --- & \textcolor{blue}{0.XX} & \textcolor{blue}{0.XX} & \textcolor{blue}{0.XX} & --- \\
\midrule
\multicolumn{9}{@{}l}{\textit{SSL methods}} \\
& TS2Vec~\citep{yue2022ts2vec} & --- & --- & --- & \textcolor{blue}{0.XX} & \textcolor{blue}{0.XX} & \textcolor{blue}{0.XX} & --- \\
& PatchTST~\citep{nie2023patchtst} & --- & --- & --- & \textcolor{blue}{0.XX} & \textcolor{blue}{0.XX} & \textcolor{blue}{0.XX} & --- \\
& MTS-JEPA~\citep{mtsjepa2026} & --- & --- & --- & \textcolor{blue}{0.XX} & \textcolor{blue}{0.XX} & \textcolor{blue}{0.XX} & --- \\
& AE-LSTM~\citep{lecam2025aelstm}$^\dagger$ & 13.99 & --- & --- & --- & --- & --- & --- \\
\midrule
\rowcolor{blue!5}
& \textbf{Traj JEPA E2E} & \textbf{14.23}$_{\pm0.4}$ & \textbf{15.37}$_{\pm0.9}$ & \textbf{25.62}$_{\pm0.3}$ & \textcolor{blue}{\textbf{0.XX}} & \textcolor{blue}{\textbf{0.XX}} & \textcolor{blue}{\textbf{0.XX}} & \textcolor{blue}{\textbf{X.XX}} \\
\rowcolor{blue!5}
& \textbf{Traj JEPA Frozen} & 17.81$_{\pm1.7}$ & 19.25$_{\pm3.2}$ & 29.35$_{\pm0.6}$ & \textcolor{blue}{0.XX} & \textcolor{blue}{0.XX} & \textcolor{blue}{0.XX} & \textcolor{blue}{X.XX} \\
\bottomrule
\end{tabular}}% end resizebox
\end{table}
}

The key observation: \textbf{every baseline fills at most two event-type columns}. Supervised RUL methods (STAR, TTSNet) have no anomaly capability. Anomaly detectors (Anomaly Transformer, DCdetector) have no RUL capability. SSL methods (TS2Vec, PatchTST, MTS-JEPA) were evaluated only on anomaly benchmarks. Trajectory JEPA fills all columns because the pretraining objective is event-type agnostic.
...
\paragraph{Full-sequence target.} Replacing the future-only target with a full-sequence target ($\x_{1:t+k}$) improves the frozen probe by $-2.1$ RMSE on FD001 at 100\% labels, with 8$\times$ lower seed variance. The improvement generalizes to FD003 ($-0.9$) and FD004 ($-1.3$).
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Hand-engineered baseline margin:} $+5.0$ RMSE over a 58-feature Ridge regressor.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Gap to supervised SOTA.} JEPA E2E (14.23) vs.\ STAR (12.19) on FD001 at 100\% labels. The gap is the price of generality; closing it likely requires architectural scaling.
...
\label{app:fd002}
FD002 (6 operating conditions) reveals a distribution shift: the frozen encoder achieves val RMSE 15.35 but test RMSE 26.07. Conditions 1, 2, and 5 are $2.5$--$3.1\times$ overrepresented at test time.
