...
\;=:\;C_\eta\,L^2\,\varepsilon.
\end{equation}
The constant is now $C_\eta=(2\underline\eta(1-\overline\eta))^{-1}$
in terms of the \emph{posterior} bounds, not the marginal bounds. By
Jensen, $\underline\eta\le p\le\overline\eta$, so
$C_\eta\ge C_p$; the bound becomes \emph{looser} but
\emph{provably valid}. \CONF

\paragraph{When the two coincide.}
$C_\eta=C_p$ exactly when $\eta(H^*)$ has support equal to
$\{\underline p,\overline p\}$ (a knife-edge that never happens in
practice). In all realistic cases $C_\eta$ is the right
...
$I(H_t;E)\ge I(H^*;E) - C_\eta L^2\varepsilon$.

\paragraph{Verification.}
Step 1 gives $I(H_t;E)\ge I(\hat H;E)$. Step 5 (with the A1$'$ fix)
combined with Step 6 and A2 gives
$I(H^*;E)-I(\hat H;E)\le C_\eta L^2\varepsilon$. Substituting:
\[
I(H_t;E)\ge I(\hat H;E)\ge I(H^*;E)-C_\eta L^2\varepsilon. \quad\square
\]
\CONF
...
7. Final assembly                  & \CONF & with new constant $C_\eta=1/(2\underline\eta(1-\overline\eta))$. \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Status.} A1$'$ has been added as a numbered assumption in \texttt{theory\_main.tex} and \texttt{theory\_appendix.tex}, and the bound constant has been updated from $C_p$ to $C_\eta = (2\underline\eta(1-\overline\eta))^{-1}$ throughout.
...
Corollary 1 (Precursor Necessity). The only inconclusive row is MSL ($n=1$ seed in v28
...
over a hypothesis class $\mathcal F$ of head functions $h\to[0,1]$ on
$n$ labelled samples. Suppose $\mathcal F$ has Rademacher complexity
$\mathfrak R_n(\mathcal F)$ and is rich enough to realise the Bayes
predictor on $H_t$. Then with probability $\ge 1-\delta$ over the labels,
\begin{equation}\label{eq:excess}
\E\bigl[(\hat\eta_n(H_t)-\eta_t(H_t))^2\bigr]
\;\le\;
\underbrace{2\bigl(\mathbb{H}(E)-\alpha\bigr)\ln 2}_{\text{representation gap}}
\;+\;
\underbrace{4\,\mathfrak R_n(\mathcal F)+\sqrt{\tfrac{2\ln(2/\delta)}{n}}}_{\text{statistical error}},
\end{equation}
where $\eta_t(h)\coloneqq\PP(E=1\mid H_t=h)$.
\end{theorem}

\begin{proof}[Proof sketch]
The Bayes risk for binary BCE is $\mathbb{H}(E\mid H_t)\ln 2$, and
$\mathbb{H}(E)-\mathbb{H}(E\mid H_t)=I(H_t;E)\ge\alpha$. The first term in
...
predictor). The MSE bound on the left-hand side follows from the standard relationship between BCE calibration error and squared error via Pinsker's inequality; this step introduces a constant factor that we absorb into the representation-gap term.
The second term is the standard Rademacher uniform
...
representation-gap term $\mathbb{H}(E)-\alpha$. As long as the encoder is good
...
$\mathfrak R_n(\mathcal F)+\sqrt{(2\ln(2/\delta))/n}$ exceeds the
representation gap. For the v20 5\%-labels result on FD001
($n\approx 5\cdot 10^3$, MLP head), $\mathfrak R_n\sim 1/\sqrt n\sim 0.014$,
which is now comparable to the representation gap $\mathbb{H}(E)-\alpha$ for an
encoder with $\alpha$ close to $\mathbb{H}(E)$. This is the regime where pretraining
...
$C_\eta(\Delta t)=(2\underline\eta_{\Delta t}(1-\overline\eta_{\Delta t}))^{-1}$
...
\end{itemize}
The product $C_\eta L^2\varepsilon$ therefore grows superlinearly
...
against per-horizon labels $E_{t+\Delta t_k}$.\footnote{The BCE training objective treats each horizon independently; the resulting $\hat\lambda_k$ therefore approximates the marginal $\PP(E_{t+\Delta t_k}=1\mid H_t)$ rather than the conditional hazard. The CDF parameterization enforces monotonicity regardless.} Define the cumulative
...
$\mathfrak R_n(\mathcal F)$ exceeds the representation gap.}
...
\item \WEAK\ \textbf{The constant $C_\eta$ is data-dependent.}
The fix in \S\ref{sec:phase6a} replaces $C_p$ with $C_\eta$ in
terms of the support of $\eta(H^*)$, which is itself a learned object.
A worst-case version using $\eta(h)\in[0,1]$ trivially gives
$C_\eta=2$ but loses tightness. A proof that
$C_\eta$ is bounded by a function of A2 and A3 alone is still
open.
...
in-paper proof.
\item \CONF\ The qualitative prediction of Proposition 1 + Corollary 1 (Precursor Necessity)
