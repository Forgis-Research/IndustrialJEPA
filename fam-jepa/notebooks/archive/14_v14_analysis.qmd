...
### Phase 2d: Full-sequence on FD003 (POSITIVE multi-subset generalization)

In-domain pretrain on FD003 (fan + HPC, 100 engines), 3 seeds frozen + E2E @ 100%:
...
Full-sequence on FD003: frozen -0.86 RMSE vs V2 (7× lower std), E2E **-1.70 RMSE** - new best JEPA E2E on FD003. Direct contrast with cross-sensor (Phase 3d): cross-sensor REGRESSES +5.2 RMSE on FD003.

Full-sequence is the more universally useful V14 architectural change - it generalizes across fault modes while cross-sensor is FD001-specific.

### Phase 2e: Full-sequence on FD004 (hardest subset)

FD004 has 6 operating conditions + fan + HPC faults. 249 train engines, per-condition normalization, 3 seeds frozen + E2E @ 100%:
...
Full-sequence FD004: frozen -1.27, E2E -0.35. Smaller absolute improvement than FD001/FD003, consistent with FD004's gap being a per-condition distribution shift rather than an architecture issue.
...
Full-sequence uniformly improves frozen probe across all three subsets (-2.1, -0.9, -1.3), and is best-or-tied on E2E. Universally positive generalization.

## Cross-Sensor Attention (Phase 3) {#sec-crosssensor}

Sensor-as-token encoder inspired by iTransformer + STAR. 14 tokens per cycle, alternating temporal causal + cross-sensor attention, 961K params. Pretrain + fine-tune sweep:

| Budget | V2 frozen      | Phase 2 (full-seq) | Phase 3 (cross-sensor) |
|:-------|:---------------|:-------------------|:------------------------|
| 100%   | 17.81 ± 1.7    | 15.70 ± 0.21       | **14.98 ± 0.22**        |
| 20%    | 19.83 ± 0.30   | **17.20 ± 1.91**   | 25.02 ± 10.19           |
| 10%    | 19.93 ± 0.90   | **18.79 ± 1.96**   | 22.05 ± 3.59            |
| 5%     | **21.53 ± 2.0**| 26.57 ± 4.70       | 27.40 ± 7.44            |

Cross-sensor is the best frozen result at 100% labels and reduces the gap to supervised STAR from 5.6 to 2.8 RMSE. E2E at 100% is indistinguishable from V2 (14.35 vs 14.23). At low labels, cross-sensor is brittle: the 20% std of 10.19 is driven by one catastrophic seed.

**Attention map finding (physics-aligned)**: during degradation, many temperature and pressure sensors (s2, s3, s4, s11, s15) concentrate attention on **s14 (core speed Nc)**. The top five healthy-to-degradation attention shifts all point at s14:

- s3 (HPC outlet temp) → s14: +0.103
- s15 (bypass ratio) → s14: +0.099
- s11 (HPC outlet pressure) → s14: +0.096
- s4 (LPT outlet temp) → s14: +0.095
- s2 (LPC outlet temp) → s14: +0.090

Concentration on a single key (core speed) is consistent with degradation being a near-one-dimensional phenomenon in attention space - the same qualitative finding as the PCA analysis in v12 (PC1 dominates, 47.6% variance).

![Healthy phase attention](../analysis/plots/v14/cross_sensor_attention_healthy_mean.png)

![Degradation phase attention](../analysis/plots/v14/cross_sensor_attention_degradation_mean.png)

![Degradation minus healthy (shift)](../analysis/plots/v14/cross_sensor_attention_diff.png)

### Phase 3d: Does this generalize to FD003?

Tested cross-sensor architecture with in-domain FD003 pretrain (fan + HPC faults, 100 engines).

| Subset | V2 frozen       | Cross-sensor frozen | Delta    |
|:-------|:----------------|:---------------------|:---------|
| FD001  | 17.81 ± 1.7     | 14.98 ± 0.22         | -2.83    |
| FD003  | 19.25 ± 3.2     | **24.49 ± 0.12**     | **+5.24** |

**Cross-sensor REGRESSES on FD003.** The architectural gain is FD001-specific.

**Attention pattern differs by fault mode**:

| Pattern               | FD001 (fan-only)         | FD003 (fan + HPC)        |
|:----------------------|:-------------------------|:-------------------------|
| Biggest attention key | s14 (corrected Nc)       | s8 (Nf), s13 (NRf)       |
| Shift sign            | POSITIVE (focus on s14)  | NEGATIVE (defocus)       |
| Physics               | core speed as reference  | fan-speed sensors change |

Both subsets concentrate attention on rotor-speed sensors (s8 Nf, s13 NRf, s14 NRc), but the specific sensor and direction differ by fault mode. The broad finding - "cross-sensor attention is rotor-speed centric during degradation" - generalizes. The specific FD001 pattern (s*→s14 positive) does not. Paper now reports cross-sensor as an FD001-specific finding.

![FD003 healthy attention](../analysis/plots/v14/cross_sensor_fd003_attention_healthy_mean.png)

![FD003 degradation attention](../analysis/plots/v14/cross_sensor_fd003_attention_degradation_mean.png)

![FD003 shift](../analysis/plots/v14/cross_sensor_fd003_attention_diff.png)
...
## AE-LSTM Head-to-Head Replication (Phase 5b.3) {#sec-aelstm}

Replicated AE-LSTM (LeCam et al. 2025) on our pipeline: FC autoencoder (14→64→32→32→64→14) + LSTM regressor (hidden=64) with joint fine-tuning. Matched splits, seeds, RUL cap. 5 seeds.

| Method                                | FD001 RMSE    | Seed std |
|:--------------------------------------|:--------------|:---------|
| AE-LSTM (paper, best-of-28-configs)   | 13.99         | ---      |
| **AE-LSTM (our 5-seed replication)**  | **16.07**     | ±2.80    |
| **Trajectory JEPA E2E (5 seeds)**     | **14.23**     | ±0.39    |

Per-seed AE-LSTM: [13.81, 21.23, 14.88, 13.71, 16.73]. The best seed (13.71) matches the paper's 13.99 - consistent with best-of-grid interpretation.

**Under matched statistical reporting, Trajectory JEPA beats AE-LSTM by 1.84 RMSE with 7× lower seed variance.** The published 13.99 is a grid-search tail, not a central tendency. This settles the SSL comparability question that Phase 5b's audit raised.
