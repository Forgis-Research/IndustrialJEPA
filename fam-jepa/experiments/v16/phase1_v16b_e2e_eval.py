"""
V16b E2E Evaluation.
Same as phase1_v16a_e2e_eval.py but loads V16b checkpoints.
V16b uses the same V16aJEPA architecture (imported from phase1_v16a.py).
"""
...
def eval_e2e_fixed(model, data, seed=42, lr=3e-4, n_epochs=100):
    ...
    targets = np.concatenate(targets)  # Already raw cycles
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("V16b E2E Evaluation (same architecture as V16a)")
    print("=" * 60)

    data = load_cmapss_subset('FD001')

    results = {
        'description': 'V16b E2E evaluation with best checkpoint per seed',
        'seeds': SEEDS,
        'e2e_rmse_per_seed': [],
        'val_rmse_per_seed': [],
    }
    ...
    for seed in SEEDS:
        ...
        with open(V16_DIR / 'phase1_v16b_e2e_results.json', 'w') as f:
            json.dump(results, f, indent=2)

    valid = [x for x in results['e2e_rmse_per_seed'] if x is not None]
    if valid:
        results['e2e_mean'] = float(np.mean(valid))
        results['e2e_std'] = float(np.std(valid))
        print(f"\n  Mean E2E: {results['e2e_mean']:.2f} +/- {results['e2e_std']:.2f}")
        print(f"  V2 E2E baseline: 14.23 +/- 0.39")

    with open(V16_DIR / 'phase1_v16b_e2e_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to {V16_DIR / 'phase1_v16b_e2e_results.json'}")
