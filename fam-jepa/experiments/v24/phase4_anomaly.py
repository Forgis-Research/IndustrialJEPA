"""V24 Phase 4/5: anomaly datasets pretrain + pred-FT (SMAP, MSL, PSM, SMD, MBA).

Each dataset is a separate run. Usage:
  python phase4_anomaly.py --dataset SMAP
  python phase4_anomaly.py --dataset MSL
  ...

Pretrain on per-entity (or per-stream) normal data.
Pred-FT on intra-entity (entity splits) or chronological (single stream).
"""
...
# Horizons for anomaly datasets (per ARCHITECTURE.md)
ANOMALY_HORIZONS = [1, 5, 10, 20, 50, 100, 150, 200]


# ---------------------------------------------------------------------------
# Dataset-specific loaders (return: pretrain_sequences, ft_splits, n_channels)
# ---------------------------------------------------------------------------
...
def load_psm():
    """PSM: single stream, chronological split with gap."""
    from data.psm import load_psm
    data = load_psm(normalize=False)
    train = data['train'].astype(np.float32)        # (~132K, n_channels)
    ...
    pretrain_sequences = {0: train}
    return pretrain_sequences, ft, train.shape[1]
...
def load_mba():
    """MBA: single-stream cardiac, chronological with gap."""
...
# ---------------------------------------------------------------------------
# Build EventDataset over anomaly entities
# ---------------------------------------------------------------------------
...
def run_seed(dataset: str, seed: int, max_context: int = 512,
    ...
    if ckpt_path.exists():
        ...
        for k, seq in pretrain_seqs.items():
            ...
            if L - cut >= 128:
                val_seqs[k] = seq[cut:]
        if not val_seqs:
            val_seqs = pretrain_seqs  # fallback
    ...
    save_surface(surf_path, eval_out['p_surface'], eval_out['y_surface'],
                 ...
                 metadata={'dataset': dataset, 'seed': seed, 'phase': 'v24_p4'})
...
def main():
    ...
    for seed in args.seeds:
        ...
        if res is not None:
            results.append(res)
        out = args.out or (RES_DIR / f'phase4_{args.dataset}.json')
