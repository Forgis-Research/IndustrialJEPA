"""V26 Phase 8: PA-F1 / non-PA F1 from v26 surfaces.

Reuses anomaly_metrics() from evaluation.grey_swan_metrics.
Reports per-dataset PA-F1 + non-PA-F1 aggregated over 3 seeds.
"""
...
OUT_PATH = ROOT / 'results' / 'phase8_pa_f1.json'
...
def main():
    ...
    for ds in DATASETS:
        ...
        if seed_results:
            ...
            print(f"  {ds} aggregate: PA-F1={np.mean(pas):.3f}+/-"
                  f"{np.std(pas):.3f}  non-PA-F1={np.mean(nonpas):.3f}+/-"
                  f"{np.std(nonpas):.3f}", flush=True)
