"""V26 Phase 2: C-MAPSS FD001/002/003 with hazard CDF parameterization.

Protocol (same as v24 Phase 2, just routed to v26 dirs):
...
- Model:    imports FAM from model.py — hazard CDF via cumprod

Target (vs v24 Phase 2):
  FD001 AUPRC: 0.9257 +/- 0.0008 — expect >= 0.90 (stop if below)
  FD002 AUPRC: 0.9081 +/- 0.0019
  FD003 AUPRC: 0.7658 +/- 0.0085
"""
...
def run_seed(subset: str, seed: int, max_context: int = 512,
    ...
    print(f"  pretrain samples: train={len(train_pre)}, val={len(val_pre)}",
          flush=True)
    tloader = DataLoader(train_pre, batch_size=64, shuffle=True,
                         collate_fn=collate_pretrain, num_workers=0)
    vloader = DataLoader(val_pre, batch_size=64, shuffle=False,
                         collate_fn=collate_pretrain, num_workers=0)
    ...
    if ckpt_path.exists():
        ...
        t0 = time.time()
        pre_out = pretrain(model, tloader, vloader, lr=3e-4,
        ...
        print(f"  pretrain done in {pre_time:.1f}s "
              f"(best_val={pre_best:.4f})", flush=True)
    ...
    save_surface(surf_path, eval_out['p_surface'], eval_out['y_surface'],
                 ...
                 metadata={'subset': subset, 'seed': seed, 'phase': 'v26_p2'})
...
def main():
    ...
    for seed in args.seeds:
        ...
        with open(out, 'w') as f:
            json.dump({'subset': args.subset, 'results': results}, f,
                      indent=2)
        # Enforce the 0.90 floor on FD001
        if args.subset == 'FD001' and res['auprc'] < 0.90:
            print(f"  *** ABORT: FD001 AUPRC {res['auprc']:.4f} < 0.90 ***",
                  flush=True)
            sys.exit(2)
    ...
    for r in results:
        print(f"  seed {r['seed']:4d}: AUPRC={r['auprc']:.4f}  "
              f"AUROC={r['auroc']:.4f}  "
              f"mono={r['monotonicity_violation_rate']:.6f}", flush=True)
