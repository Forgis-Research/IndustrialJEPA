"""
V14 Phase 2d: full-sequence target encoder on FD003.

Counterpart to Phase 3d (cross-sensor on FD003). Tests whether the
Phase 2 architectural win (full-sequence improves frozen -2.1 on FD001)
generalizes to FD003, where Phase 3 regressed.

Protocol: in-domain pretrain on FD003 with full-sequence target,
3-seed frozen + E2E fine-tune at 100% labels.

Output: experiments/v14/phase2d_fd003_results.json
"""
...
def pretrain(data, ckpt_path, log_path, seed=42):
    ...
    if HAS_WANDB:
        try:
            run = wandb.init(project='industrialjepa', name='v14-phase2d-fullseq-fd003',
                             tags=['v14-phase2d-fullseq-fd003'], reinit=True)
        except Exception: pass
    t0 = time.time()
    with open(log_path, 'w') as f: f.write("V14 Phase 2d FD003 full-sequence\n")
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        if epoch % PROBE_EVERY == 0 or epoch == 1:
            ...
            if probe_rmse < best:
                ...
                torch.save(best_state, ckpt_path); no = 0
            else: no += 1
...
def main():
    print(f"V14 Phase 2d: full-sequence on FD003")
    t0 = time.time()
    ...
    ckpt = V14_DIR / 'best_pretrain_full_sequence_fd003.pt'
    log = V14_DIR / 'phase2d_output.log'
    ...
    # V2 FD003 baselines (from paper tab:multisubset): frozen 19.25 +/- 3.2, e2e 15.37 +/- 0.9
    res['v2_fd003'] = {'frozen_mean': 19.25, 'frozen_std': 3.2,
                        'e2e_mean': 15.37, 'e2e_std': 0.9}
    res['cross_sensor_fd003_frozen'] = 24.49

    res['wall_time_s'] = time.time() - t0
    print(f"\n=== FD003 SUMMARY ===")
    print(f"Full-seq frozen: {res['frozen_mean']:.3f} +/- {res['frozen_std']:.3f} "
          f"(V2: 19.25, cross-sensor: 24.49)")
    print(f"Full-seq E2E:    {res['e2e_mean']:.3f} +/- {res['e2e_std']:.3f} "
          f"(V2: 15.37)")
    print(f"Total wall time: {(time.time()-t0)/60:.1f} min")

    with open(V14_DIR / 'phase2d_fd003_results.json', 'w') as f:
